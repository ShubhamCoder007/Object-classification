...
class Object_classifier:
	...
	def __init__(self, window):
		...
		self.window = window
		self.window.wm_title("Object Classification")
		
		l1 = Label(window, text = "Enter the obj name")
		...
		b2 = Button(window, text = "Capture obj face", width = 12, command = self.capture_face)
	...
	def create_directory(self):
		...
		self.list1.insert(END,"Object profile created")
	...
	def load(self):
		...
		self.classifier.load_weights('weight_object.hdf5')
	...
	def save(self):
		...
		self.classifier.save_weights('weight_object.hdf5', overwrite = True)
	...
	def train(self):
		...
		self.classifier.fit_generator(training_set,
								 steps_per_epoch = self.compute_files()*64,
								 epochs = 2,
								 validation_data = test_set,
								 validation_steps = self.compute_files()*16)
								 
								 
		self.categories = training_set.class_indices					
		with open("object_labels.txt",'w') as file:
	...
	def detect(self):
		...
		self.list1.insert(END,"Object: ")
		self.list1.insert(END, self.categories[i])
	
	
	
	def dic_load(self):
		f = open("object_labels.txt")
		a = f.read()
		a = a[1:-1]
		a=a.split(sep=',')
		dic = {}
